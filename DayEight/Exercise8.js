...
dog.getDogInfo = function(){
    return `The dog's name is ${dog.name}, it has ${dog.legs} legs, It is ${dog.color}, it is ${dog.age} years old, 
    it knows how to ${this.bark()}, and it is a ${dog.breed}.`
}
...
//Exercise: Level 2 
//Question 1
//Find the array with the most elements? skills.length
...
for (let key in users) {
    if (users[key].skills.length > length){
        length = users[key].skills.length
        maxSkill = key
    }
}
...
for (let key in users) {
    users[key].isLoggedIn?count++:null
}
 console.log(count)
 ...
 for (let key in users) {
    users[key].points>=50?countUsers++:null
 }
 ...
 for (let key in users) {
    let mernSet = new Set([...users[key].skills, 'MongoDB', 'Express', 'React', 'Node'])
    if (mernSet.size == users[key].skills.length){
        mernStack.push(key)
    }
 }
 ...
 // Question 4
users.Yetty = {
    email: 'yetty@yetty.com',
    skills: ['HTML', 'CSS', 'JavaScript', 'Java', 'PHP', 'Ruby'],
    age: 30,
    isLoggedIn: false,
    points: 50
}
